...
def acc_moons(y):
    ...
    for i in len(moon_gravity): # determines index of which moons to take out
        if moon_gravity[i] == 'IO':
            which_moons = [which_moons, 0]
        if moon_gravity[i] == 'EUROPA':
            which_moons = [which_moons, 1]
        if moon_gravity[i] == 'GANYMEDE':
            which_moons = [which_moons, 2]
        if moon_gravity[i] == 'CALLISTO':
            which_moons = [which_moons, 3]
    print(which_moons)
    ...
    accs = np.delete(accs, which_moons, 0) # deletes the acceleration vectors from the moons that are not considered for gravitational influence on the particle
...
def plot_figs(yps, tps, accps, orbitps, kounts, t1, row, particleID, script_ID):
    ...
    axes.plot_wireframe(yps_inert[1, 0, kounts[0]-1, 0] + sphere_1_x, yps_inert[1, 1, kounts[0]-1, 0] + sphere_1_y, yps_inert[1, 2, kounts[0]-1, 0] + sphere_1_z, color = 'k')
    axes.plot_wireframe(yps_inert[2, 0, kounts[0]-1, 0] + sphere_2_x, yps_inert[2, 1, kounts[0]-1, 0] + sphere_2_y, yps_inert[2, 2, kounts[0]-1, 0] + sphere_2_z, color = 'k')
    axes.plot_wireframe(yps_inert[3, 0, kounts[0]-1, 0] + sphere_3_x, yps_inert[3, 1, kounts[0]-1, 0] + sphere_3_y, yps_inert[3, 2, kounts[0]-1, 0] + sphere_3_z, color = 'k')
    axes.plot_wireframe(yps_inert[4, 0, kounts[0]-1, 0] + sphere_4_x, yps_inert[4, 1, kounts[0]-1, 0] + sphere_4_y, yps_inert[4, 2, kounts[0]-1, 0] + sphere_4_z, color = 'k')

    axes.set_xlabel('X-axis [km]')
    axes.set_ylabel('Y-axis [km]')
    ...
    plt.plot(disc_0_x, disc_0_y, c='k') # Jupiter
    plt.plot(yps_inert[1, 0, kounts[0] - 1, 0] + disc_1_x, yps_inert[1, 1, kounts[0] - 1, 0] + disc_1_y, c='k')
    plt.plot(yps_inert[2, 0, kounts[0] - 1, 0] + disc_2_x, yps_inert[2, 1, kounts[0] - 1, 0] + disc_2_y, c='k')
    plt.plot(yps_inert[3, 0, kounts[0] - 1, 0] + disc_3_x, yps_inert[3, 1, kounts[0] - 1, 0] + disc_3_y, c='k')
    plt.plot(yps_inert[4, 0, kounts[0] - 1, 0] + disc_4_x, yps_inert[4, 1, kounts[0] - 1, 0] + disc_4_y, c='k')
    ...
    plt.plot(disc_0_x, disc_0_y, c='k')  # Jupiter
    plt.plot(yps_inert[1, 0, kounts[0] - 1, 0] + disc_1_x, yps_inert[1, 2, kounts[0] - 1, 0] + disc_1_y, c='k')
    plt.plot(yps_inert[2, 0, kounts[0] - 1, 0] + disc_2_x, yps_inert[2, 2, kounts[0] - 1, 0] + disc_2_y, c='k')
    plt.plot(yps_inert[3, 0, kounts[0] - 1, 0] + disc_3_x, yps_inert[3, 2, kounts[0] - 1, 0] + disc_3_y, c='k')
    plt.plot(yps_inert[4, 0, kounts[0] - 1, 0] + disc_4_x, yps_inert[4, 2, kounts[0] - 1, 0] + disc_4_y, c='k')
    ...
    plt.plot(disc_0_x, disc_0_y, c='k')  # Jupiter
    plt.plot(yps_inert[1, 1, kounts[0] - 1, 0] + disc_1_x, yps_inert[1, 2, kounts[0] - 1, 0] + disc_1_y, c='k')
    plt.plot(yps_inert[2, 1, kounts[0] - 1, 0] + disc_2_x, yps_inert[2, 2, kounts[0] - 1, 0] + disc_2_y, c='k')
    plt.plot(yps_inert[3, 1, kounts[0] - 1, 0] + disc_3_x, yps_inert[3, 2, kounts[0] - 1, 0] + disc_3_y, c='k')
    plt.plot(yps_inert[4, 1, kounts[0] - 1, 0] + disc_4_x, yps_inert[4, 2, kounts[0] - 1, 0] + disc_4_y, c='k')

    plt.xlabel('Y-axis [km]')
    plt.ylabel('Z-axis [km]')
    ...
    handle3, = plt.plot((tps[particleID, 0:kounts[particleID]] - t1) / 3600, acc_mag[2,:], label='Centrifugal force (fictitious)', c='g')
    handle4, = plt.plot((tps[particleID, 0:kounts[particleID]] - t1) / 3600, acc_mag[3,:], label='Coriolis force (fictitious)', c='c')
    ...
    handle7, = plt.plot((tps[particleID, 0:kounts[particleID]] - t1) / 3600, acc_mag[6,:], label='Plasma drag', c='0.75')
    handle8, = plt.plot((tps[particleID, 0:kounts[particleID]] - t1) / 3600, acc_mag[7,:], label='Gravity of moons', c='k')
    plt.legend(handles=[handle1, handle2, handle3, handle4, handle5, handle6, handle7, handle8], loc = 'best')
    plt.xlabel('Time [hours]')
...
def load_initial_conditions(ic_file):
    ...
    for i in range(num_particles):
        ...
        W = np.array([0., 0., w_jup])  # angular velocity vector
        v_orb_rot = spice_inert2rot(np.append(pos_inert, v_orb_inert), t1)[3:] # v_orb_inert - np.cross(W, pos_inert)
